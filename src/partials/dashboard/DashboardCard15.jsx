  ...
  useEffect(() => {
    const fetchUsersCount = async () => {
      try {
        ...
        const usersCount = response.data.filter(user => user.role === 'HrAgent');
    ...
    <div className="flex flex-col col-span-full h-40 sm:col-span-6 xl:col-span-4 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
      <div className="px-5 pt-5">
        <header className="flex justify-between items-start mb-2">
          ...
          <User size={24} strokeWidth={1.5} width="32" height="32" alt="Icon 03"/>  
        ...
        <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">Nombre d'agents RH </h2>
        <div className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase mb-1">Nombre d'agents RH </div>
